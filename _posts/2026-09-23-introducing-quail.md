...
date: 2026-09-23
author: "Shreya Shankar, Charles Frye, Fergus Finn, Arnav Dhariya, Joseph Barrow, and Meryem Arik"
permalink: /blog/introducing-quail/
description: "Quail jointly plans AI-SQL queries and model inference to reduce KV recomputation, CPU overhead, and GPU cost."
image: /assets/blog/introducing-quail/quail-throughput-by-dataset.png
---

For the big picture, read our companion post, *Hitting a Billion Tokens per Minute on One GPU* (link coming soon). This post covers the more boring technical details of how Quail plans and runs AI-SQL queries.

<!-- TODO: Add the companion post URL after publication. -->

<aside class="tldr"><strong>TL;DR:</strong> Most AI-SQL work (i.e., AI functions in SQL) today relies on costly, closed LLM APIs. Moving these workloads to open-weight models allows us to jointly optimize query planning and model inference. Quail, the QUery-Aware Inference Layer, is an open-source AI-SQL execution engine we are building. Across 29 QUAIL-B queries, Quail has a 1.84x average speedup over well-tuned vLLM. Try it out!</aside>

# 1. AI-SQL makes unstructured data useful, but it is expensive.

SQL remains our best language for asking questions over structured data,
but databases have never handled messy text very well. Now, with AI-SQL,
also called AI functions, users can express what they want in natural
language and evaluate it over rows of unstructured text. Major database
vendors now support AI-SQL, including Snowflake Cortex AISQL, BigQuery
AI functions, and Databricks AI Functions.

An AI function evaluates the natural-language instruction row by row.
One SQL query can therefore create hundreds of thousands or millions of
model calls. For example, a filter over *N* rows requires *N* LLM
calls, while a naive join between tables *A* and *B*, with *n*<sub>A</sub>
and *n*<sub>B</sub> rows, requires *n*<sub>A</sub>*n*<sub>B</sub> LLM calls.

The database community has proposed a number of logical optimizations
that reduce the number of LLM calls, for example, by pushing filters
below joins, reordering predicates, choosing cheaper implementations, or
pruning irrelevant LLM calls [\[1\]](https://arxiv.org/abs/2512.02289)
[\[2\]](https://arxiv.org/abs/2505.14661)
[\[3\]](https://arxiv.org/abs/2407.11418)
[\[4\]](https://arxiv.org/abs/2512.05399)
[\[5\]](https://cloud.google.com/blog/products/data-analytics/more-than-100x-faster-and-cheaper-llm-powered-sql-queries-with-proxy-models).
But still, the resulting query plans can end up needing hundreds of
thousands, or even millions, of LLM calls.

# 2. Query plans should control LLM inference.
...
<figure>
  <img src="{{ '/assets/blog/introducing-quail/bio4-logical-plan.png' | relative_url }}" alt="The BIO-4 logical query plan.">
  <figcaption>Figure 2. A logical query plan for the BIO-4 query plan filters all three inputs before the joins. Both joins use the medical report as the anchor (i.e., first document in the prompt).</figcaption>
</figure>

*Given a dataset of medical reports and a dataset of possible adverse
reactions, find serious adverse event reports that mention both a
cardiovascular reaction and a neurological reaction.<sup><a href="#note-1">1</a>, <a href="#note-2">2</a></sup>*
...
two joins). The logical plan, shown in Figure 2, works as follows:
...
separate inference request. For the filters, there's just one document
...
prompts. We’d also order operators such that the filter on reports is
computed right before the first join; to maximize KV reuse.
...
one H100. We give it enough batch capacity to use the GPU. The query
takes *4 hours*, or over 16 times the SoL estimate! There are two
reasons for the inefficiency:

- The primary reason is host overhead. In a profile of BIO-4 at scale
  factor 0.1, GPU kernels run for only 27.51% of the first join’s wall
  time. Figure 3 shows a representative 3.5-second window: the CPU
  spends long stretches scheduling and tracking requests while the H100
  waits.<sup><a href="#note-4">4</a></sup>

- The second is *KV regret:* vLLM sometimes discards KV that it needs
  again later. On BIO-4, it therefore processes 174.6 million tokens
  instead of 124.3 million: 50.3 million extra tokens, or 40% more work.

<figure>
  <img src="{{ '/assets/blog/introducing-quail/vllm-bio4-perfetto.jpg' | relative_url }}" alt="A PyTorch profiler trace of the first BIO-4 join in stock vLLM.">
  <figcaption>Figure 3. A 3.5-second window from the first BIO-4 join from a PyTorch profiler trace. The CPU main thread alternates between vLLM scheduler work and execute_context. GPU stream 25 runs during the three execute_context blocks; as you can see, the gaps between them leave the H100 idle.</figcaption>
</figure>
...
## 3.1 You can run your first Quail query in a few lines of Python.
...
Our current evaluation focuses on the first two goals. Quail uses
DeepGEMM, FlashAttention 3, and fused Triton kernels, but we do not yet
measure MFU or tune the core matrix multiplication and attention
kernels. We defer a full MFU study to future work.

As shown in Figure 4, Quail consists of a query frontend, a query
...
analytical query engine. ~~Users~~ We, and you, can add new query
operators, planning rules, execution backends, models, or support for
other hardware.

<figure>
  <img src="{{ '/assets/blog/introducing-quail/quail-architecture.png' | relative_url }}" alt="The Quail architecture.">
  <figcaption>Figure 4. Quail turns Arrow data and an AI-SQL query into a logical plan. The planner applies SQL rewrites, lowers the AI operations into physical operators, and plans operator pipelines for KV reuse. The execution engine runs the physical plan and executes its AI operations on the GPU.</figcaption>
</figure>
...
Users can specify the model and GPU count. Quail currently supports 3
models: Qwen3 4B FP8, Qwen3 32B FP8, and Diffusion Gemma on H100 GPUs,
but other models and hardware can be added through the extension
interface.
...
**Database query optimizers already know how to push down filters,
reorder predicates, and choose join order. AI-SQL adds several new
...
4.  *Perform Inference-specific query rewrites.* We lower AI operations
    into physical operators and plan pipelines that preserve reusable
    KV.
...
<figure>
  <img src="{{ '/assets/blog/introducing-quail/quail-physical-plan.png' | relative_url }}" alt="The physical plan and execution path for BIO-4.">
  <figcaption>Figure 5. Quail lowers BIO-4 to the physical plan on the left. On the right, one CPU worker runs its physical operators and manages KV. Each AI physical operator invokes Quail’s inference program on the GPU.</figcaption>
</figure>
...
hardware backends. How does vLLM *do it all*? As shown in Figure 6,
...
<figure>
  <img src="{{ '/assets/blog/introducing-quail/vllm-kernel-selection.png' | relative_url }}" alt="How vLLM builds a model forward pass.">
  <figcaption>Figure 6. vLLM builds the model’s forward pass for the selected model and GPU through two paths: compiling PyTorch operations into GPU kernels and selecting prewritten kernels for specialized operations.</figcaption>
</figure>
...
KV once. Figure 7 shows how Quail evaluates attention in two parts. One
...
<figure>
  <img src="{{ '/assets/blog/introducing-quail/quail-join-attention.png' | relative_url }}" alt="How Quail evaluates attention for an AI join.">
...
**Quail is faster than stock vLLM on 27 of the 29 QUAIL-B queries. The
geometric mean speedup is 1.84x, and the maximum speedup is 11.22x on
BIO-2. The two queries where stock vLLM wins expose one missing feature
clearly: Quail does not yet reuse matching prefixes across different
rows.**

## 4.1 We compare Quail with stock vLLM on one H100.
...
joins, we also report “pipelined vLLM.” It pipelines requests between
consecutive filters and between consecutive joins. Both baselines use
vLLM 0.26.0 with automatic prefix caching. We use the largest stable
settings: 25,305 maximum batched tokens, 4,096 sequences, GPU memory
utilization of 0.91, and one CUDA graph for 8,192 tokens. Larger
settings ran out of GPU memory.

## 4.2 Quail has a 1.84x speedup over stock vLLM.

The table averages tokens/second, KV regret, and cost per query within
each dataset at scale factor 0.1. Cost multipliers are relative to
Quail.
...
<td style="text-align: left;">Quail</td>
<td style="text-align: left;">Stock vLLM</td>
</tr>
<tr>
...
Figure 1 summarizes throughput by dataset, and Figure 8 reports latency
...
<figure>
  ...
  <figcaption>Figure 1. Average requested input tokens per second on QUAIL-B, shown as a percentage of the Speed-of-Light estimate (i.e., theoretical hardware limits) for each dataset. We use Qwen3 4B FP8 and one H100.</figcaption>
</figure>

Figure 9 focuses on the eight queries where pipelining changes how vLLM
...
## 4.3 Quail runs BIO-4 14.04x faster than stock vLLM.
...
*Figure 10. BIO-4 results at scale factor 1.0. GPU cost excludes model
startup. SoL values are estimates.*
...
## 4.4 Stock vLLM runs AGENT-1 2.32x faster than Quail.
...
so stock vLLM wins. For example, two rows might look like the following:
...
Stock vLLM finishes AGENT-1 in 103.07 seconds, 2.32x faster than Quail’s
239.12 seconds. The SoL estimate is 47.47 seconds, so stock vLLM still
...
## 4.5 Quail supports DiffusionGemma for batch AI workflows.
...
fast. On IMDB-2, DiffusionGemma matched 88.89% of the Qwen3 32B
reference answers, compared with 76.41% for Qwen3 4B. It ran the query
in 32.41 seconds, compared with 21.20 seconds for Qwen3 4B, on one H100.
...
and Shreya Shankar, *Maximizing perf on AI-SQL queries with the
KV-optimal left-deep join*.

# 5. We are just getting started with Quail!
...
**Hash join meets de-RoPE.** Today, Quail reuses an anchor’s KV within
...
now, please try Quail! If these ideas sound interesting, reach out to
...
dataset](https://aclanthology.org/2023.findings-emnlp.896/).

<span id="note-2"><strong>2.</strong></span> The SQL form of BIO-4 is shown below.
...
<span id="note-3"><strong>3.</strong></span> The speed of light estimate assumes 100 percent model FLOP/s
utilization (MFU), so every forward pass sustains peak GPU arithmetic
throughput. Real systems cannot reach that rate, but higher MFU still
helps. We do not yet measure Quail's MFU.
